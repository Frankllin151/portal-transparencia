<x-guest-layout>
    ...
    <div class="max-w-464-px mx-auto w-100">
        ...
        @if ($errors->any() || session('error'))
            <div class="alert alert-danger mb-20 text-center">
                @if (session('error'))
                    {{ session('error') }}
                @else
                    {{-- Se não for um erro 'session' (erro de autenticação), exibe a primeira mensagem de validação --}}
                    {{ $errors->first() }}
                @endif
            </div>
        @endif
        {{-- FIM DO BLOCO DE MENSAGEM GERAL --}}
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div class="icon-field mb-16">
                ...
                <input type="email"
                       name="email"
                       id="email"
                       class="form-control h-56-px bg-neutral-50 radius-12" {{-- REMOVIDO: @error('email') is-invalid @enderror --}}
                       placeholder="Email"
                       value="{{ old('email') }}"
                       required autofocus>
                {{-- REMOVIDO: @error('email') <div class="invalid-feedback">{{ $message }}</div> @enderror --}}
            </div>
            ...
            <div class="position-relative mb-20">
                <div class="icon-field">
                    ...
                    <input type="password"
                           name="password"
                           id="password"
                           class="form-control h-56-px bg-neutral-50 radius-12" {{-- REMOVIDO: @error('password') is-invalid @enderror --}}
                           placeholder="Senha"
                           required>
                </div>
                ...
                {{-- REMOVIDO: @error('password') <div class="invalid-feedback">{{ $message }}</div> @enderror --}}
            </div>
