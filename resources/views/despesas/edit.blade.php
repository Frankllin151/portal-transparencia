...
<div class="card">
  ...
  <select name="tipo_recurso" class="form-select" required>
    ...
    @endforeach
          ...
          <textarea name="observacoes" id="observacoes" class="form-control" rows="3" placeholder="Digite observações adicionais">{{ old('observacoes', $editarDespesa->observacoes ?? '') }}</textarea>
        </div>
      </div>
            </div>
          </div>
