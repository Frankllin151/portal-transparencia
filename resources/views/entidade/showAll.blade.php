          ...
          <tr>
           ...
           <td>{{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y') }}</td>
            <td>
              ...
              <a href="{{route("entidade.edit", $item->id)}}" class="w-32-px h-32-px bg-success-focus text-success-main rounded-circle d-inline-flex align-items-center justify-content-center" title="Editar">
              ...
              <form id="delete-form-{{ $item->id }}"
                    action="{{route("entidade.destroy", $item->id)}}" {{-- Rota de delete --}}
                    method="POST"
                    style="display: none;">
                @csrf
                @method('DELETE')
